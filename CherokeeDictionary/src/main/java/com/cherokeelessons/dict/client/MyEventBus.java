...
public class MyEventBus extends EventBus {
	 ...
	 */
	private final List<Command> deferredDeltas=new ArrayList<>();
	...
	private <H> void _doFire(Event<H> event, Object source) {
		if (event == null) {
		...
		try {
			...
			GWT.log("doFire: "+event.toDebugString());
			...
			while (it.hasNext()) {
				H handler = it.next();
				if (handler==null) {
					GWT.log("Null Handler");
					continue;
				}
				GWT.log("Handler: "+handler.getClass().getSimpleName());
				try {
					GWT.log("dispatching start: "+event.getClass().getSimpleName()+"=>"+handler.getClass().getSimpleName());
					dispatchEvent(event, handler);
					GWT.log("dispatching done: "+event.getClass().getSimpleName()+"=>"+handler.getClass().getSimpleName());
				} catch (Throwable e) {
					if (causes == null) {
			...
			if (causes != null) {
				for (Throwable cause: causes) {
					GWT.log("Event Failure: "+cause.getMessage(), cause);
				}
